...
from project_panel.dispatcher_client import DispatcherClient
from project_panel.rmf_msg_observer import AsyncRmfMsgObserver, RmfMsgType
...
# Note: Get Dashboard Config for each "World", specific to PROJECT impl
...
def main(args=None):
    ...
    if "PROJECT_API_SERVER_IP" in os.environ:
        server_ip = os.environ['PROJECT_API_SERVER_IP']
        print(f"Set Server IP to: {server_ip}")

    if "PROJECT_API_SERVER_PORT" in os.environ:
        port_num = int(os.environ['PROJECT_API_SERVER_PORT'])
    ...
    print(f"Starting PROJECT API Server: {server_ip}:{port_num}, "
